...
def get_id_prediction(arb_index):
    file = 'temp_drop_attack_0.01.txt'  # read frames collected by frame fetcher which collects frames from a file in the datasets
...
while True:
    start=time.time()
    counter = counter + 1
    next_initial_time = frames_fetcher.fetch(initial_time,attack_type,attack_freq, duration=1, data_lines=all_packets)
